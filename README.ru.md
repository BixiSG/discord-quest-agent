...
- Сам переподключается после перезагрузки клиента и возвращает отладочный порт, если его
  снесло автообновление Discord. Не умирает, когда вы закрываете Discord: запустите его
  снова как угодно — агент перезапустит его с портом и вернётся примерно за минуту.
...
| Любая строка в *Пропущенных* | **Скрыть из списка** (вернуть можно в Настройки > Сервис) |
...
К сообщению внутри Discord можно добавить короткий **звук**: переключатель **Звук** и
ползунок **громкости** прямо под ним, сдвиг ползунка сразу даёт послушать. Ошибки без
звука.
...
### Что нового и куда писать

В начале настроек есть блок **О программе**. **Что нового** открывает список изменений
([`CHANGELOG.md`](CHANGELOG.md), он приходит с каждым обновлением): по блоку на версию,
новые сверху, установленная помечена. После самообновления агент один раз показывает
всплывающее сообщение, а на шестерёнке держится точка, пока список не открыт, так что об
изменениях узнаёте без похода в репозиторий. Рядом кнопки **GitHub** (открывает проект в
браузере) и **Сообщить о проблеме** (открывает issues). **Папка установки** открывает в
Проводнике папку с `Uninstall.bat`, `config.json` и файлами агента. Список изменений
...
1. Скачайте [последний релиз](../../releases/latest) (v1.4.2 или новее) или *Code -> Download ZIP*.
...
Если Discord с отладочным портом на вашем ПК уже запускает что-то другое (скрипт-лаунчер,
другой инструмент), ставьте через `Install.bat -AttachOnly`: тогда агент только
подключается к этому Discord и сам никогда его не запускает и не перезапускает.
...
**Нет кнопки в заголовке окна.** Обычно агент просто не запущен (проверьте в диспетчере
задач: PowerShell с `QuestAgent.ps1`), и Discord поднялся без отладочного порта;
запустите агента ярлыком из меню «Пуск». Пока агент работает, он ловит запущенный
вручную Discord и перезапускает его с портом. Если квесты при этом выполняются
